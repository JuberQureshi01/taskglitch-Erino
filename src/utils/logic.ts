...
export function computeROI(revenue: number, timeTaken: number): number | null {
  //  BUG 5 FIX
  // The original code was `return revenue / (timeTaken as number);`
  // This was unsafe and caused "Infinity" if timeTaken was 0,
  // or NaN if inputs were invalid.

  // Validate that inputs are actual, finite numbers
  if (
    typeof revenue !== "number" || typeof timeTaken !== "number" || !Number.isFinite(revenue) || !Number.isFinite(timeTaken)
  ) {
    return null; // Return null for invalid inputs
  }

  // Validate that timeTaken is a positive number
  // Division by zero (or negative time) is not a valid ROI
  if (timeTaken <= 0) {
    return null; // Return null
  }

  // If checks pass, perform the safe calculation
...
export function sortTasks(tasks: ReadonlyArray<DerivedTask>): DerivedTask[] {
  return [...tasks].sort((a, b) => {
    ...
    // return Math.random() < 0.5 ? -1 : 1;  // this causing random order
    return a.title.localeCompare(b.title); //BUG FIX 3: Now even when the roi and priorityWeight are equal the order will be consistent based on title alphabetically sorting
  });
}
