...
interface UseTasksState {
  ...
  undoDelete: () => void;
  // BUG 2 FIX Add clearLastDeleted to the interface 
...
export function useTasks(): UseTasksState {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        //   ];
        // } // This block was added to intentionally cause the ROI bug
  ...
  // BUG 1 FIX: Double Fetch Issue we have already fetches all the tasks above so this is redundant
...
const undoDelete = useCallback(() => {
  ...
  //BUG 2 FIX: Add this new function 
