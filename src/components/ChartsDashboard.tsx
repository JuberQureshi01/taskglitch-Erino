...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  const roiBuckets = [
    // BUG 5 FIX (Chart) 
    // The original code `(t.roi as number) < 200` was true for `null`,
    // putting N/A tasks in the wrong bucket.
    // We must explicitly check that roi is not null.
