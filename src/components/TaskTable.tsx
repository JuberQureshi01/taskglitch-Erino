...
export default function TaskTable({ tasks, onAdd, onUpdate, onDelete }: Props) {
  ...
  const handleEditClick = (e:React.MouseEvent<HTMLButtonElement, MouseEvent>,task: Task) => {
    //bug fixed 4: prevent row click event when clicking edit button
    e.stopPropagation();
    setEditing(task);
    setOpenForm(true);
  };
  ...
  const handleDelete=(e:React.MouseEvent<HTMLButtonElement, MouseEvent>,id:string)=>{
    //bug fixed 4: prevent row click event when clicking delete button
    e.stopPropagation();
    onDelete(id);
  }
                  ...
                  <TableCell align="right">
                    <Stack direction="row" spacing={1} justifyContent="flex-end">
                      ...
                      <Tooltip title="Delete">
                        <IconButton onClick={(e) => handleDelete(e,t.id)} size="small" color="error">
